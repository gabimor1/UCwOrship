...
def create_arabic_song_image(song_data, params):
                    ...
                    for segment in segments:
                        if not re.fullmatch(r"\[.*?\]", segment):
                            ...
                            shaped_substring = reshaped_clean_line[
                                ...
                                - segment_len
                                + occurrence_count
                                + occurrence_count_allah : reshaped_lyric_cursor
                            ]
                            ...
                            reshaped_lyric_cursor -= (
                                segment_len - occurrence_count - occurrence_count_allah
                            )
                            ...
                            if x_calculator < chord_size_mid:
                                assert last_accord_end is not None
                                draw.text(
                                    (last_accord_end, y_position),
                                    chord_text,
                                    font=chord_font,
                                    fill=chord_color,
                                    anchor="ra",
    ...
    final_image_scaled = img.crop(
        (image_width_scaled - croping_redundant_area, 0, image_width_scaled, y_position)
    )
