...
class SongSheetApp(tk.Tk):
    ...
    def _create_controls_panel(self):
        ...
        ttk.Label(all_songs_frame, text="All Songs", font=("Helvetica", 14, "bold")).grid(
        ...
        add_button = ttk.Button(
            all_songs_frame, text="Add to Session List ↓", command=self._add_to_session
        )
        ...
        ttk.Label(session_list_frame, text="Session List", font=("Helvetica", 14, "bold")).grid(
            row=0, column=0, columnspan=2, pady=5
        )
        ...
        ttk.Label(bottom_controls_frame, text="Controls", font=("Helvetica", 14, "bold")).pack(
            pady=5
        )
        self._create_slider(
            bottom_controls_frame, "Lyrics Font Size", self.params["lyric_font_size"], 10, 80
        )
        self._create_slider(
            bottom_controls_frame, "Chords Font Size", self.params["chord_font_size"], 8, 50
        )
        ttk.Checkbutton(
        ...
        self._create_stepper(
            bottom_controls_frame, "Capo", self.params["capo"], self.on_capo_change
        )
        self._create_stepper(
            bottom_controls_frame, "Scale", self.params["scale_steps"], self.on_scale_change
        )
        ...
        ttk.Button(action_frame, text="Set as Default", command=self.set_as_default).pack(
    ...
    def _create_image_panel(self):
        ...
        self.image_canvas.bind(
            "<Double-Button-1>", self._reset_zoom
        )  # New event for resetting zoom
    ...
    def load_song_files(self):
        try:
            ...
            self.song_listbox.insert(tk.END, "'txt_files' directory not found.")
    ...
    def _parse_song_file(self, file_path):
        ...
        for line in lines:
            ...
            if line.startswith("Title:"):
                ...
                try:
                    self.params["capo"].set(int(line.replace("Capo:", "").strip()))
                except ValueError:
    ...
    def _display_on_canvas_after_delay(self, pil_img, canvas_widget):
        ...
        # New logic: Check if this is the projector and it's zoomed
        # The main view always gets a thumbnail
        if canvas_widget == self.projector_label and self.is_zoomed:
