...
def create_arabic_song_image(
    text_file_path, output_image_path, arabic_font_path, arabic_bold_font_path, chord_font_path=None
):
    """
    ...
    try:
        ...
        # section_title_font = ImageFont.truetype(
        #     arabic_bold_font_path, section_title_font_size * scale_factor
        # )
        capo_font = ImageFont.truetype(arabic_font_path, capo_font_size * scale_factor)
        chord_font = (
            ImageFont.truetype(chord_font_path, chord_font_size * scale_factor)
            if chord_font_path
    ...
    for line in lines:
        ...
        if line.startswith("Title:"):
            ...
            song_data.append(
                {"type": "capo", "content": f"Capo: {line.replace('Capo:', '').strip()}"}
            )
        elif re.fullmatch(
            r"\[.*?\]", line
        ):  # A line containing ONLY a section marker like [Verse 1]
    ...
    for section in song_data:
        ...
        if sec_type == "lyrics_section":
            for line in section["lines"]:
                ...
                max_line_size = max(max_line_size, total_lyric_width)
    ...
    for section in song_data:
        ...
        if sec_type in ["title", "capo"]:
            ...
            # Track maximum width
            x_size = max(x_size, text_width)
            x_pos = image_width_scaled - text_width - padding_scaled
            draw.text((x_pos, y_position), bidi_text, font=font, fill=text_color)
            y_position += font.getbbox(bidi_text)[3] + (
                section_spacing_scaled if sec_type == "capo" else line_spacing_scaled
            )
        ...
        # Draw Lyric Sections (Verses, Choruses)
            ...
            for line in section["lines"]:
                ...
                draw.text(
                    (image_width_scaled - padding_scaled - offset_to_center, lyric_y_pos),
                    reshaped_clean_line,
                ...
                x_calculator = (
                    image_width_scaled - padding_scaled - offset_to_center
                )  # Start calculating from the right margin.
                reshaped_lyric_cursor = len(
                    reshaped_clean_line
                )  # This tracks our position in the pre-shaped string

                # Track maximum width
                x_size = max(x_size, total_lyric_width)
                for segment in segments:
                    if not re.fullmatch(r"\[.*?\]", segment):  # It's a lyric segment
                        ...
                        shaped_substring = reshaped_clean_line[
                            reshaped_lyric_cursor - segment_len : reshaped_lyric_cursor
                        ][::-1]
